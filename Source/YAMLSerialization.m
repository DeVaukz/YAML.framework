...
static int __YAMLSerializationParserInputReadHandler(void *data, unsigned char *buffer, size_t size, size_t *size_read) {
...
static int __YAMLSerializationEmitterOutputWriteHandler (void *data, unsigned char *buffer, size_t size) {
  return ([((__bridge NSOutputStream *) data) write: buffer maxLength: size] > 0);
}

static int __YAMLSerializationAddObject (yaml_document_t *document, id value) {
  int result = 0;
